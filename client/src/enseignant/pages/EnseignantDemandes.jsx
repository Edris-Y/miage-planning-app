...
export default function EnseignantDemandes() {
  ...
  const filtered = useMemo(() => {
    ...
    return demandes.filter((d) => {
      const haystack = normalise([d.type, d.date, d.cohorte, d.salle, d.statut].join(' '));
            ...
            <table className="ens-table">
              ...
              <tbody>
                {filtered.map(d => {
                  ...
                  const isAttente = normalise(d.statut).includes('attente');
                  return (
                    <tr key={d.id}>
                      <td>
                        <span style={{
                      ...
                      <td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          {isAttente && (
                            <button className="ens-btn-ghost" style={{ fontSize: '.82rem', color: 'var(--primary)', fontWeight: 700, padding: '4px 8px' }}>
                              Modifier
                            </button>
                          )}
                          <button
